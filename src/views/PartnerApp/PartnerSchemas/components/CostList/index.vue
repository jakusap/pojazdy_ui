<template>
  <div class="cost-list__wrapper">
    <div class="cost-list__header">
      <h2>{{ $t('Schemas.additional_costs') }}</h2>
    ...
    <ul class="cost-list">
      <li v-for="(listElement, id) in editedList" :key="id">
        <el-form :id="`costForm${id}`" :rules="costRules" :model="listElement" class="cost-list__element">
          ...
          <el-form-item prop="provider">
            <el-select v-model="listElement.provider" :disabled="!editMode">
              <el-option
                v-for="(provider, idKey) in supportedProviders"
                :key="idKey"
...
export default {
  ...
  methods: {
    addListElement() {
      this.editedList.unshift({ name: '', placeholder: this.$t('additional_cost'), amount: 0, provider: null });
    },
